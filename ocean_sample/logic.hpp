...
public:
    ...
    { // возвращение координат в океане
        return {x, y};
    };
    ы virtual Entity *tick(std::unordered_map<std::pair<int, int>, Entity> *M) = 0; // update a life cycle
...
class PriorityQueueMap
{
    ...
    auto static cmp = [](std::pair<T, Priority> a, std::pair<T, Priority> b)
...
public:
    ...
    */
    ...
    std::pair<int, int> partner_is_near(std::unordered_map<std::pair<int, int>, Entity> *M, std::pair<int, int> size)
    { // only good_carp can be a partner
    ...
    // std::pair<int, int>
    Entity *reproduce(std::pair<int, int> parent_2, std::unordered_map<std::pair<int, int>, Entity> *M, std::pair<int, int> size)
    { // use coo
